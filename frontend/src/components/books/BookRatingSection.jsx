...
export default function BookRatingSection({ bookId, bookSlug }) {
  ...
  // Fetch book reviews
  const fetchBookReviews = async (bookSlug, page = 1, pageSize = 5) => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_URL}/api/books/${bookSlug}/reviews/?page=${page}&page_size=${pageSize}`);
  ...
  // Submit book review
  const submitBookReview = async (bookSlug, score, comment) => {
    try {
      const response = await axios.post(`${API_URL}/api/books/${bookSlug}/reviews/`, {
        score,
        comment
      });
  ...
  useEffect(() => {
    if (bookSlug) {
      fetchBookReviews(bookSlug, currentPage, reviewsPerPage);
    }
  }, [bookSlug, currentPage, reviewsPerPage]);
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      await submitBookReview(bookSlug, reviewForm.score, reviewForm.comment);
                  ...
                  <Form onSubmit={handleSubmitReview}>
                    <Form.Group className="mb-3">
                      ...
                      <div className="star-rating d-flex align-items-center">
                        ...
                        <span className="ms-2 text-muted">({reviewForm.score} of 5)</span>
                      </div>
                    </Form.Group>
                    <Form.Group className="mb-3">
